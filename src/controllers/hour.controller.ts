...
export class HourController {
	...
	public async shutdown(): Promise<void> {
		...
		switch (shouldShutdown) {
			case true:
				...
				break;
			case false:
				this._logger.info("Shutdown skipped: current time is within allowed hours.");	
				break;
			default:
				this._logger.error("An error occurred while checking shutdown conditions. Switch is in default case (unpossibly)");
				break;
		}
	}
    ...
    const interval = (day === "Saturday" || day === "Sunday") 
	...
	private async getCurrentTime(region: string): Promise<TimeApiResponse | null> {
		try {
			const response = await axios.get<TimeApiResponse>(`https://timeapi.io/api/Time/current/zone?timeZone=${region}`);
			return response.data;
		} catch (e: any) {
			this._logger.error(`Failed to fetch time data from TimeAPI: ${e.message}`);
      ...
      try {
          ...
          const fallbackResponse: TimeApiResponse = {
              ...
              dayOfWeek: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][now.getDay()] as Day,
              dstActive: false
          };
